...
(
    id   uuid        not null primary key,
    name varchar(40) not null unique -- название участка
...
(
    ...
    CONSTRAINT unique_postman_region UNIQUE (postman_id, region_id) -- Уникальная пара postman_id и region_id
);
...
(
    ...
    street_id   uuid       not null REFERENCES Streets (id), -- id, внешний ключ к таблице улицы
    building    varchar(5) not null,                           -- номер дома
    sub_address varchar(5)        null                                -- дополнительный столбец, номер квартиры, может быть null
...
(
    id   uuid        not null primary key,
    name varchar(10) not null unique -- вид издания газета/журнал
);
